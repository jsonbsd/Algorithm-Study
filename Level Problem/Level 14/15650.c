...
static void
backtrack(int *digits, bool *valid, int level, int value, int ndigit, int len) {
    int i;
    ...
    if (level == len) {
        ...
        for (i = value; i < ndigit; ++i) {
            if (valid[i]) {
                digits[level] = i + 1;
                valid[i] = false;
                backtrack(digits, valid, level + 1, i + 1, ndigit, len);
                valid[i] = true;
            }
        }
    }
...
int main(int argc, const char *argv[]) {
    ...
    backtrack(digits, valid, 0, 0, ndigit, len);
